...
class SceneTreeContextMenu:
	...
	func _popup_menu(paths: PackedStringArray) -> void:
		...
		if not paths.is_empty():
			...
			add_context_menu_item("Copy the NodePaths of Node and its Family Tree", _on_copy_node_tree_all, nodepath_icon)
			...
			add_context_menu_item("Copy the NodePaths of Node and its Family Tree With Node Types", _on_copy_node_tree_with_types_full, nodepath_icon)
			add_context_menu_item("Copy the NodePaths of Node and its Children (1st gen) With Node Types", _on_copy_node_tree_with_types_direct, nodepath_icon)
	...
	func _on_copy_node_tree_with_types_full(paths: PackedStringArray) -> void:
		if paths.is_empty():
			return
		...
		# Generate the tree structure including all descendants with types
		var tree_text = _generate_full_tree_with_types(root_node, "", true)

		# Copy to clipboard
		DisplayServer.clipboard_set(tree_text.strip_edges())

		print("Copied node and entire family tree with types to clipboard")

	func _on_copy_node_tree_with_types_direct(paths: PackedStringArray) -> void:
		if paths.is_empty():
			return
		...
		# Generate the tree structure with only direct children with types
		var tree_text = _generate_direct_children_tree_with_types(root_node, true)

		# Copy to clipboard
		DisplayServer.clipboard_set(tree_text.strip_edges())

		print("Copied node and direct children with types to clipboard")
	...
	# Generate a full tree structure with all descendants including node types
	func _generate_full_tree_with_types(node: Node, prefix: String = "", is_last: bool = true) -> String:
		var result = ""
		
		# Add current node with type and proper prefix
		var node_info = node.name + " [" + node.get_class() + "]"
		if prefix == "":
			result += node_info + "\n"
		elif prefix != "":
			result += prefix + node_info + "\n"
		...
		for i in range(child_count):
			...
			result += _generate_full_tree_with_types(child, new_prefix, is_last_child)
	...
	# Generate a tree structure with only direct children including node types
	func _generate_direct_children_tree_with_types(node: Node, exclude_internal: bool) -> String:
		var result = ""

		# Add current node with type
		result += node.name + " [" + node.get_class() + "]" + "\n"
		...
		# Add direct children with tree structure and types
		for i in range(child_count):
			var child = children[i]
			var is_last = (i == child_count - 1)
			
			var child_info = child.name + " [" + child.get_class() + "]"
